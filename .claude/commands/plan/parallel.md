...
4. Analyze the codebase by reading `AGENTS.md` and relevant docs.
   **ONLY PERFORM THIS FOLLOWING STEP IF docs are not available or older than 3 days**: Use `/scout <instructions>` slash command to search the codebase for files needed to complete the task.
...
- Group component/hook/store work into separate phases when possible
- Separate infrastructure setup from application logic
- Isolate different feature domains (e.g., timeline vs export vs auth)
- Split by package/directory (e.g., apps/web vs packages/ui)
- Create independent test phases per feature area

**Phase Organization Example:**
```
Phase 01: Store & Type Definitions (can run independently)
Phase 02: Component Implementation (can run independently)
Phase 03: Hook & Integration (can run independently)
