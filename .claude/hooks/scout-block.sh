...
# Reads patterns from .ckignore file (defaults: node_modules, __pycache__, .git, dist, build, .next, .turbo)
#
# Blocking Rules:
...
#   - Blocked: cd node_modules, ls .next/, cat dist/file.js
#   - Allowed: npm build, bun build, next build
...
try {
  ...
  let blockedPatterns = ['node_modules', '__pycache__', '.git', 'dist', 'build', '.next', '.turbo']; // defaults
...
if [ "$CHECK_RESULT" = "BLOCKED" ]; then
  echo "ERROR: Blocked directory pattern (node_modules, __pycache__, .git/, dist/, build/, .next/, .turbo/)" >&2
  exit 2
fi
